...
profit_new as( SELECT
      ...
      ext_user_id,
      fact_payment,
      month_payment as month_NR,
      ifnull(SA_profit.SA_NR, 0) + ifnull(SS_profit.SS_NR, 0) AS NR_new
    FROM payment
    LEFT JOIN SA_profit 
...
profit_all as (SELECT profit_new.*,
      ifnull(SA_profit_all.SA_NR, 0) + ifnull(SS_profit_all.SS_NR, 0) AS NR_all
    FROM profit_new
    LEFT JOIN SA_profit_all 
      ON profit_new.ext_user_id = SA_profit_all.user_id AND substr(profit_new.month_NR,1,7)=SA_profit_all.month
    LEFT JOIN SS_profit_all
      ON profit_new.ext_user_id = SS_profit_all.user_id AND substr(profit_new.month_NR,1,7)=SS_profit_all.month)
